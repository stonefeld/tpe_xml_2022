...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="season_data">
    ...
    ### Competition
    <xsl:value-of select="season/competition" />
    #### Year:
    <xsl:value-of select="season/date/year" />
    . From
    <xsl:value-of select="season/date/start" />
    to
...
| --- | --- | --- |
|
          <xsl:value-of select="//season_data/stages/stage/@start_date" />
          |
          <xsl:value-of select="local/score" />
          |
          <xsl:value-of select="visitor/score" />
          |
| |
